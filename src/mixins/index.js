...
export default {
  ...
  methods: {
    ...
    syncOfflineFarmersData() {
      ...
      if (this.newFarmersOffline.length > 0) {
        this.newFarmersOffline.forEach(function(farmer) {
          if (
            farmer.fingerprint !== 'no-photo.jpg' &&
            farmer.photo !== 'no-photo.jpg'
          ) {
            self.dataURLtoFile(
              farmer.fingerprint,
              farmer.fingerprintFileName,
              'fingerPrint'
            );
    ...
    getBase64(url) {
      this.toDataBase64(url, function(dataUrl) {
        return dataUrl;
      });
    },
    addNewData(farmerData) {
      farmersService
        ...
        .catch(() => {
          this.$store.dispatch('emptyFarmerData');
          this.errorMessage(
            'Failed to save farmer offline, some information may be invalid.'
          );
        });
    },
    checkInternet() {
    ...
    logout() {
      ...
      authService
        .logout()
        .then(() => {
          this.successMessage('User Logout');
        })
        .catch(errors => {
